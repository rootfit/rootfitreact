...
const sql = {
  ...
  addOrder: 'INSERT INTO orderTBL (id, prodNum, name, email, nickname, addr, phone, price) values (?, ?, ?, ?, ?, ?, ?, ?)'  // 주문 추가
};
...
const OrderDAO = {
  ...
  // 주문상품 DB 추가
  addOrder: async (id, prodNum, name, email, nickname, addr, phone, price, callback) => {
    let conn = null;
    try {
      const pool = getPool();
      ...
      const [results] = await conn.query(sql.addOrder, [id, prodNum, name, email, nickname, addr, phone, price]);
